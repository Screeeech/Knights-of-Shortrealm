...
func _is_interacting(area: InteractionArea, event: InputEvent) -> String:
    for i in range(area.interact_actions):
        if event.is_action_pressed(area.interact_actions[i]):
            return area.interact_actions[i]
